...
import { DevFluent } from "@devfluent/ai";

function App() {
  const [code, setCode] = useState("");

  const handleAICompletion = async () => {
    const suggestion = await DevFluent.complete(code);
    setCode(suggestion);
  };
...
export default function Hero() {
  ...
  const handleAISuggestion = async () => {
    const suggestion = await DevFluent.suggest("hero component");
    return suggestion;
  };
...
export default function Navbar() {
  ...
  const handleSearch = async () => {
    const results = await DevFluent.search(searchQuery);
    return results;
  };
